...
export const CSPProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const handleViolation = (event) => {
      ...
      // Add to local violations (luôn update local trước, không phụ thuộc backend)
      const violation = {
      ...
      // Update nonce từ violation (luôn check và update nếu different, ưu tiên log đỏ)
      if (event.originalPolicy) {
        ...
        if (match && match[1] !== window.__cspNonce) {
          ...
          // Dispatch event để dashboard update nonce nếu cần (e.g., refresh display)
          window.dispatchEvent(new CustomEvent("csp-nonce-updated", { detail: { nonce: match[1] } }));
        }
      }
      // Send to backend (fire-and-forget, không block local update nếu fail)
      const backendUrl =
    ...
    window.getCSPNonce = () => {
      ...
      // Method 3: Extract từ violations đã có (ưu tiên log đỏ)
